...
import { LocalCacheHelper } from 'Assemblies/Caching/Roblox.Caching/Legacy/LocalCache/Helper';
...
(async (args: Record<string, any>) => {
	...
	EphemeralCountersServiceServer.set('views', [__baseDirName + '/Views/ASPX', __baseDirName + '/Views/Roblox.EphemeralCounters.Service']);
	...
	await (async () => {
		...
		SystemSDK.ServerStarter(VersionCompatibilityServiceServer, Hosts['VersionCompatibilityService'], true, true, httpPort, httpsPort);
		...
		SystemSDK.ServerStarter(EphemeralCountersServiceServer, Hosts['EphemeralCountersService'], !isNginx, true, httpPort, httpsPort);
		...
		SystemSDK.ServerStarter(ClientTelementryServiceServer, Hosts['ClientTelementryServiceHost'], true, true, httpPort, httpsPort);
		...
		SystemSDK.ServerStarter(EconomyCreatorStatsApiServer, Hosts['EconomyCreatorStatsHost'], !isNginx, true, httpPort, httpsPort);
		SystemSDK.ServerStarter(EngagementPayoutsServiceServer, Hosts['EngagementPayoutsHost'], !isNginx, true, httpPort, httpsPort);
		...
		SystemSDK.ServerStarter(MetricsInternalWebsiteServer, Hosts['MetricsInternalWebsiteHost'], !isNginx, true, httpPort, httpsPort);
...
process.stdin.on('data', function (key: string) {
	...
	if (key === 't' || key === 'T') {
		return FASTLOGS(DFLog('Tasks'), "The current time is '%s'", new Date(Date.now()).toUTCString());
	}

	if (key === 'u' || key === 'U') {
		return FASTLOGS(DFLog('Tasks'), '[DFLog::Tasks] The current process uptime is: %ss', process.uptime().toFixed(7));
	}
